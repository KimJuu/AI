...
public class NeuralNetwork
{
	...
	static final double LEARNING_RATE = 0.017345345;
	...
	public NeuralNetwork backpropError(
			double targetResult,
			int outputNo)
	{
		...
		for (int i = INPUT_NEURONS + HIDDEN_NEURONS; i < INPUT_NEURONS + HIDDEN_NEURONS + OUTPUT_NEURONS; i++)
		{
			...
			if (i == (outputNo + INPUT_NEURONS + HIDDEN_NEURONS))
			{
				...
				{
					neurons[i].getWeights()[INPUT_NEURONS + HIDDEN_NEURONS - j - 1] = neurons[i].getWeights()[INPUT_NEURONS + HIDDEN_NEURONS - j - 1] + LEARNING_RATE * neurons[i].getError() * neurons[(HIDDEN_NEURONS + INPUT_NEURONS - 1) - j + INPUT_NEURONS].getOutput();
				}
			}
		}
